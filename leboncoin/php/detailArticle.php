	...
	try{
		...
		while ($articles = $requete->fetch()) {
			if ($articles['id_article'] == $_GET['id_article']) {
				$user = $bdd->query('select * from membre inner join article on id=id_utilisateur where id_article='.$_GET['id_article']);
				$user = $user->fetch();
				...
				echo "	
				  ...
				  echo "<p>par <a class ='user'href='#'>".$user['pseudo']."</a></p><hr >";
