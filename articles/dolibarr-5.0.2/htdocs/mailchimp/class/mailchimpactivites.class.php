...
/**
 * \file /mailchimp/class/mailchimpactivites.php
 * \ingroup mailchimp
 ...
 */
class MailchimpActivites extends CommonObject
{
	var $db; // !< To store db handler
	var $error; // !< To return error code (or message)
	var $errors = array(); // !< To return several error codes (or messages)
	var $element = 'mailchimpactivites'; // !< Id that identify managed objects
	var $table_element = 'mailchimp_activites'; // !< Name of table without prefix where object is stored
	var $id;
	var $entity;
	var $fk_mailing;
	var $mailchimp_id;
	...
	function create($user, $notrigger = 0) {
		...
		// Insert request
		$sql = "INSERT INTO " . MAIN_DB_PREFIX . "mailchimp_activites(";
		...
		$sql .= " " . (! isset($this->mailchimp_id) ? 'NULL' : "'" . $this->db->escape($this->mailchimp_id) . "'") . ",";
		...
		if (! $error) {
			$this->id = $this->db->last_insert_id(MAIN_DB_PREFIX . "mailchimp_activites");
	...
	function fetch($id) {
		...
		$sql .= " FROM " . MAIN_DB_PREFIX . "mailchimp_activites as t";
		...
		if ($resql) {
			if ($this->db->num_rows($resql)) {
				...
				while ( $obj = $this->db->fetch_object($resql) ) {
					...
					$line = new MailchimpActivitesLineDb();
	...
	function getEmailcontactActivitesClick($sortorder = 'ASC', $sortfield = 't.rowid', $limit = 0, $offset = 0, $filter = array()) {
		...
		$sql .= " FROM " . MAIN_DB_PREFIX . "mailchimp_activites as t";
		$sql .= " INNER JOIN " . MAIN_DB_PREFIX . "mailchimp as m ON m.fk_mailing=t.fk_mailing";
	...
	function update($user = 0, $notrigger = 0) {
		...
		// Update request
		$sql = "UPDATE " . MAIN_DB_PREFIX . "mailchimp_activites SET";

		$sql .= " fk_mailing=" . (isset($this->fk_mailing) ? $this->fk_mailing : "null") . ",";
		$sql .= " mailchimp_id=" . (isset($this->mailchimp_id) ? "'" . $this->db->escape($this->mailchimp_id) . "'" : "null") . ",";
	...
	function delete($user, $notrigger = 0) {
		...
		if (! $error) {
			$sql = "DELETE FROM " . MAIN_DB_PREFIX . "mailchimp_activites";
			$sql .= " WHERE rowid=" . $this->id;
	...
	function createFromClone($fromid) {
		...
		$object = new Mailchimpactivites($this->db);
...
class MailchimpActivitesLineDb
{
	public $id;
	public $entity;
	public $fk_mailing;
