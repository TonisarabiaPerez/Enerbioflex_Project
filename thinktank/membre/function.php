...
class Bdd {
	...
	public static function connectBdd() {
		if(!self::$connexion) {
			self::$connexion = new PDO(DNS, 'energiculteur', 'EnerBIOflex2016');
...
class Inscription {
	...
	public static function profil($civilite, $nom, $prenom, $ville, $pays, $identifiant, $mail, $pass) {
		...
		$resultat = Bdd::connectBdd()->prepare(INSERT.MEMBRE.PROFIL.'; '.UPDATE.MEMBRE.NEWPROFIL);
	...
	public static function protect($login) {
		...
		$resultat = Bdd::connectBdd()->prepare(INSERT.ACCESPROFIL.INSCRIPTION);
	...
	public static function message($login) {
		...
		$resultat = Bdd::connectBdd()->prepare(INSERT.MESSAGE.BIENVENUE);
	...
	public static function passOubli($mail) {
		if(filter_var($mail, FILTER_VALIDATE_EMAIL)) {
			...
			if($verifMail -> rowCount() === 1) {
				...
				$enregistrePass = Bdd::connectBdd()->prepare(UPDATE.MEMBRE.MAJPASS.EMAIL);
